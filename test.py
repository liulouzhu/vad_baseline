...
from src.utils.tools import get_batch_mask, get_prompt_text
from src.utils.xd_detectionMAP import getDetectionMAP as dmAP
import option

def test(model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, device, logger):
    model.to(device)
    model.eval()
    ...
    logger.info("AUC1: {}, AP1: {}".format(ROC1, AP1))
    logger.info("AUC2: {}, AP2: {}".format(ROC2, AP2))

    dmap, iou = dmAP(element_logits2_stack, gtsegments, gtlabels, excludeNormal=False)
    averageMAP = 0
    for i in range(5):
        logger.info('mAP@{:.1f} = {:.2f}%'.format(iou[i], dmap[i]))
        averageMAP += dmap[i]
    averageMAP = averageMAP/(i+1)
    logger.info('average MAP: {:.2f}'.format(averageMAP))
