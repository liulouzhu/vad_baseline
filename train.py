...
from src.utils.dataset import XDDataset
from src.utils.tools import get_prompt_text, get_batch_label
import option
...
def train(model, train_loader, test_loader, args, label_map: dict, device):
    ...
    logger.info('batch size: {}, atten window: {}, visual layers: {}, visual head: {}, prompt prefix: {}, prompt postfix: {}'.format(
        args.batch_size, args.attn_window, args.visual_layers, args.visual_head, args.prompt_prefix, args.prompt_postfix))
    ...
    for e in range(args.max_epoch):
        ...
        for i, item in enumerate(train_loader):
            ...
            gt_class_idx = torch.argmax(text_labels, dim=1)
            ...
            if step % 4800 == 0 and step != 0:
                logger.info('epoch: {}, step: {}'.format(
                    e+1, step, ))
                print('| loss1: ', loss_total1 / (i+1), '| loss2: ', loss_total2 / (i+1), '| loss3: ', loss3.item())
                model.eval()
                ...
                logger.info('best ap: {}, best mAP: {}'.format(ap_best, map_best))
